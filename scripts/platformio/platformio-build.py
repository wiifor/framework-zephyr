...
def get_zephyr_modules():
    ...
    with open(west_config) as fp:
        ...
        return [m for m in config["manifest"]["projects"] if not m["path"].startswith(
            ("tools", "modules/hal"))]
...
def run_cmake():
    ...
    for m in get_zephyr_modules():
        ...
        module_path = os.path.join(env.subst("$PROJECT_PACKAGES_DIR"), module_name)
    ...
    if platform_name in PLATFORMS_WITH_EXTERNAL_HAL.keys():
        zephyr_modules.append(platform.get_package_dir(
